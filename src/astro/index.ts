  ...
  return {
    ...
    load(id) {
      if (id === resolvedVirtualModuleId) {
        return `import{createClient}from"@vrite/sdk/api";import Content from"@vrite/sdk/astro/content.astro";import*as utils from"@vrite/sdk/astro/utils";const client=createClient({token:"${options.accessToken}"}),getStaticPaths=()=>utils.getStaticPaths("${options.contentGroupId}"),getContentPieces=t=>utils.getContentPieces("${options.contentGroupId}",t);export{client,Content,getStaticPaths,getContentPieces};`;
      }
    },
  };
};
